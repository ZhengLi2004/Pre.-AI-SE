...
\email{yzy.software@gmail.com}
\email{213223455@seu.edu.cn}
\email{213221934@seu.edu.cn}
\email{213222909@seu.edu.cn}
\email{213221500@seu.edu.cn}
\email{zhujinxian@seu.edu.cn}
%%
...
\textit{This section is contributed by Hao Pang.}
...
\textit{This section is contributed by Shuoyang Xu and Pengyu Xie.}
\label{sec:3}
...
\textit{This section is contributed by Zhenyu Yan.}
...
To address these challenges, \textbf{SelfAPR} (Self-supervised Program Repair with Test Execution Diagnostics) \cite{selfapr_paper}, a novel framework, combines self-supervised learning with test execution diagnostics. SelfAPR introduces two key innovations:
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{picture/APRframework.pdf}
...
\begin{itemize}
\item \textbf{Search-based methods}: GenProg \cite{genprog}, jGenProg \cite{genprog}
...
Artificial Intelligence (AI) has revolutionized test case generation by enabling automated creation of comprehensive, context-aware test suites from diverse input sources. Leveraging machine learning (ML) and natural language processing (NLP), AI-driven frameworks now facilitate end-to-end test case generation, spanning requirement analysis, scenario design, and script generation. For instance, ML models analyze code structures, historical defect patterns, and runtime behaviors to prioritize high-risk test scenarios, while NLP engines parse unstructured requirements to extract functional specifications. Tools like \textit{Test.ai} and \textit{Functionize} exemplify this paradigm, using deep learning to automate test case generation for complex systems, including edge cases and dynamic user interactions.

The generative process unfolds through a multi-stage pipeline:
\begin{enumerate}
\item \textbf{Requirement Engineering with NLP:} AI tools employ syntactic parsing and semantic understanding to extract atomic testable units from requirement documents. This involves identifying \textit{preconditions}, \textit{action sequences}, and \textit{expected outcomes} using techniques like named entity recognition (NER) and dependency parsing. For example, an NLP engine might transform the sentence \textit{"The system must validate credit card numbers per Luhn algorithm"} into test cases covering valid/invalid formats, boundary lengths, and checksum failures.
\item \textbf{ML-Driven Scenario Enumeration:} Machine learning models, trained on historical test data, generate combinatorial test scenarios by predicting defect-prone code paths. Reinforcement learning (RL) agents can explore state spaces to identify optimal test sequences, while supervised models classify requirements into test categories (e.g., unit, integration). Tools like \textit{Sapienz} use Markov chain models to generate test cases that maximize code coverage while minimizing redundancy.
\item \textbf{Code Generation and Adaptation:} AI converts abstract test scenarios into executable scripts by leveraging template-based code synthesis or neural code generation. For instance, \textit{Applitools} generates visual testing scripts by combining image recognition with UI element locators, while \textit{Testim.io} employs self-healing mechanisms to adapt scripts to UI changes. This process often includes dynamic binding to testing frameworks (e.g., Selenium, JUnit) and integration with CI/CD pipelines.
\item \textbf{Test Suite Optimization:} AI refines generated test suites through techniques like \textit{mutation testing} (e.g., \textit{Stryker}) to evaluate fault detection capability, and \textit{test case prioritization} based on risk metrics (e.g., code change impact, historical failure rates). This optimization reduces test execution time by up to 40 percent while maintaining coverage.
\end{enumerate}

Validation of generated test cases occurs through dual mechanisms: \textit{requirement traceability} (ensuring each test case maps to specified functional points via NLP-based semantic matching) and \textit{outcome prediction} (using ML models to forecast expected results and detect anomalies). As demonstrated in Microsoft's AI-driven testing for Windows OS, this approach has substantially reduced testing cycles while improving defect detection rates. However, challenges remain in addressing \textit{model bias} (e.g., overfitting to historical data) and \textit{legacy system integration}, necessitating ongoing research in transfer learning and hybrid human-AI validation frameworks 

\paragraph{Intelligent Deployment Strategies}

AI-powered intelligent deployment strategies in software radar systems integrate evolutionary computation and adaptive optimization to address complex resource allocation challenges. The proposed approach in this paper leverages an improved genetic algorithm (GA), a prominent AI technique, to dynamically optimize component deployment for load balancing and resource efficiency.

Genetic algorithms, as a subset of AI's evolutionary computation, simulate natural selection processes to iteratively refine deployment solutions. The key AI-driven components of the strategy include:

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{Pre.-AI-SE-main/picture/develop1.jpeg}
  \caption{Genetic algorithms}
  \label{fig:adwin}
\end{figure}

\begin{itemize}
\item \textbf{Adaptive Chromosome Encoding}: Representing deployment schemes as tree-structured chromosomes, where each node denotes server-component mappings. This encoding enables AI to model hierarchical dependencies between software components (e.g., primary, secondary, tertiary levels) and physical resource constraints.

\item \textbf {Multi-objective Fitness Function}: Integrating two AI-derived metrics:
\begin {itemize}
\item Standard deviation of server resource utilization (to ensure load balance)
\item Average load threshold.
\end {itemize}
This dual-criteria mechanism allows the AI to prioritize both balance and resource sustainability, avoiding overloading or underutilization.

\item \textbf{Optimized Genetic Operations}:
\begin{itemize}
\item \textbf{Population Initialization}: Incorporating MinMin/Max-Min algorithms to filter invalid solutions (e.g., empty servers or overloaded nodes).
\item \textbf{Adaptive Crossover/Mutation}: Dynamically adjusting genetic operators based on convergence rates, reducing computational complexity from 8 30to feasible scales.
\end{itemize}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{Pre.-AI-SE-main/picture/develop2.jpeg}
  \caption{Genetic algorithms output visualization}
  \label{fig:adwin}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{Pre.-AI-SE-main/picture/develop3.jpeg}
  \caption{Genetic algorithms comprae}
  \label{fig:adwin}
\end{figure}

\item \textbf{Intelligent Termination Condition}: Employing gamma distribution analysis to terminate iterations when solution convergence is achieved (Fig. 4), reducing unnecessary computations by 20 percent compared to traditional GAs.
\end{itemize}

\textit{AI-Enhanced Scenarios Beyond the Paper}:
\begin{itemize}
\item \textbf{Reinforcement Learning Integration}: Future work could combine the GA with Q-learning to enable real-time adjustment of deployment strategies based on streaming performance data (e.g., dynamic resource spikes in radar signal processing).

\item \textbf{Neural Network-based Prediction}: Using LSTM networks to forecast component resource demands, allowing proactive deployment adjustments before load peaks (e.g., anticipating high GPU usage during radar data clustering).

\item \textbf{AutoML for Parameter Tuning}: Automatically optimizing GA parameters (crossover rate, mutation rate) via Bayesian optimization, as demonstrated in the paper's simulation with 60percent crossover and 1% mutation rates (Section IV).
\end{itemize}


Simulation results (Fig. 5) validate that the AI-driven strategy reduces server load variance by 37% and improves resource utilization by 22% compared to traditional GAs. The execution time of 15.29s showcases the efficiency of AI in handling complex deployment landscapes, where manual optimization is infeasible.
...
\begin{figure}[H]
  ...
  \caption{Output of the ADWIN algorithm illustrating abrupt concept drift. When the average error changes significantly, ADWIN shrinks the window and triggers model adjustment. Adapted from Bifet and Gavaldà.}
  \label{fig:adwin}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{Pre.-AI-SE-main/picture/components-of-kubernetes.pdf}
  \caption{K8s}
  \label{fig:adwin}
\end{figure}
...
\textit{This section is contributed by Haoyu Jia.}
...
By aggregating over all possible permutations of features, SHAP provides a globally faithful representation of how each feature influences the model’s output. This global and theoretically principled framework makes SHAP particularly useful for high-stakes domains where interpretability must align with regulatory and ethical standards. Moreover, SHAP explanations are model-agnostic and can be applied to both tree-based models (using TreeSHAP) and deep learning models (using DeepSHAP or GradientSHAP), which often build upon foundational attribution techniques like integrated gradients~\cite{Sundararajan2017}.
...
One effective method in this domain involves training a surrogate model to approximate the behavior of a black-box model within a localized region of the input space. For instance, in image classification tasks, a convolutional neural network (CNN) may be employed for high-accuracy predictions. Meanwhile, techniques like Grad-CAM can highlight discriminative regions in an image, acting as a visual surrogate to explain the CNN's focus~\cite{Selvaraju2017}. This allows developers to trace how changes in input features affect the model's classification output and identify unexpected decision boundaries that may indicate spurious correlations or overfitting.
...
Looking ahead, the role of XAI in AI testing is expected to evolve from passive interpretation to active guidance. Emerging research explores causal explainability, where models attempt to uncover not just correlations but underlying causal mechanisms that drive predictions~\cite{vonKugelgen2021}. Interactive explanation interfaces are also being developed, allowing developers and domain experts to ask ``what-if'' and ``why-not'' questions during testing and debugging~\cite{Hohman2019}. These tools enable iterative, dialogue-based model refinement that closely resembles the scientific method.
...
\textit{This section is contributed by Zheng Li and Kun Liang.}
...
\bibitem{genprog}
Le Goues, C., Nguyen, T. V., Forrest, S., et al. 
...
\bibitem{Zhou2012}
J. Zhou, H. Zhang, D. Lo.
\newblock Where should the bugs be fixed? More accurate information retrieval-based bug localization.
\newblock In \textit{Proceedings of the 34th International Conference on Software Engineering (ICSE)}, pp. 14--24, 2012.

\bibitem{Saha2013}
R. K. Saha, J. C. S. do Prado Leite, Y. G. G. de Pádua, M. T. Valente.
\newblock Improving bug localization using structured information retrieval.
\newblock In \textit{Proceedings of the 20th Working Conference on Reverse Engineering (WCRE)}, pp. 245--254, 2013.

\bibitem{Lam2015}
A. N. Lam, A. T. Nguyen, H. A. Nguyen, T. N. Nguyen.
\newblock Combining information retrieval with static analysis for bug localization.
\newblock In \textit{Proceedings of the 37th International Conference on Software Engineering (ICSE)}, pp. 416--426, 2015.

\bibitem{Huo2016}
X. Huo, M. Li, Z. M. Jiang.
\newblock Learning to rank for bug localization.
\newblock In \textit{Proceedings of the 13th Working Conference on Mining Software Repositories (MSR)}, pp. 237--247, 2016.

\bibitem{Wang2018}
W. Wang, G. Li, Z. Wang.
\newblock Bug-localization-oriented code modeling.
\newblock In \textit{Proceedings of the 26th ACM Joint Meeting on European Software Engineering Conference and Symposium on the Foundations of Software Engineering (ESEC/FSE)}, pp. 838--842, 2018.

\bibitem{Wen2019}
M. Wen, J. Li, D. Lo.
\newblock A neural approach to combining structure and text for bug localization.
\newblock In \textit{Proceedings of the IEEE International Conference on Software Maintenance and Evolution (ICSME)}, pp. 314--325, 2019.

\bibitem{Feng2020}
Z. Feng, D. Guo, D. Tang, N. Duan, X. Feng, M. Gong, L. Shou, B. Qin, T. Liu, D. Jiang, M. Zhou.
\newblock CodeBERT: A Pre-Trained Model for Programming and Natural Languages.
\newblock In \textit{Findings of the Association for Computational Linguistics: EMNLP 2020}, pp. 1536--1547, 2020.

\bibitem{Rossi2021}
B. Rossi, D. R. Firmani, F. Mercaldo, A. Santone, C. A. Visaggio.
\newblock Concept drift in software engineering.
\newblock \textit{Journal of Systems and Software}, vol. 177, art. no. 110963, 2021.
...
\bibitem{Arrieta2020}
A. B. Arrieta, N. Díaz-Rodríguez, J. Del Ser, A. Bennetot, S. Tabik, A. Barbado, S. Garcia, S. Gil-Lopez, D. Molina, R. Benjamins, R. Chatila, F. Herrera.
\newblock Explainable Artificial Intelligence (XAI): Concepts, taxonomies, opportunities and challenges toward responsible AI.
\newblock \textit{Information Fusion}, vol. 58, pp. 82--115, 2020.

\bibitem{Gilpin2018}
L. H. Gilpin, D. Bau, B. Z. Yuan, A. Bajwa, M. A. Specter, L. Kagal.
\newblock Explaining Explanations: An Overview of Interpretability of Machine Learning.
\newblock In \textit{2018 IEEE 5th International Conference on Data Science and Advanced Analytics (DSAA)}, pp. 80--89, 2018.

\bibitem{Sundararajan2017}
M. Sundararajan, A. Taly, Q. Yan.
\newblock Axiomatic Attribution for Deep Networks.
\newblock In \textit{Proceedings of the 34th International Conference on Machine Learning (ICML)}, pp. 3319--3328, 2017.

\bibitem{Barocas2019}
S. Barocas, M. Hardt, A. Narayanan.
\newblock Fairness and Machine Learning: Limitations and Opportunities.
\newblock \textit{Book available at fairmlbook.org}, 2019.
...
\newblock In \textit{Proceedings of the IEEE International Conference on Computer Vision (ICCV)}, pp. 618--626, 2017.

\bibitem{Jacovi2021}
A. Jacovi, A. Marasović, T. Miller, Y. Goldberg.
\newblock Formalizing Trust in Artificial Intelligence: Prerequisites, Causes and Goals of Human-AI Trust.
\newblock In \textit{Proceedings of the 2021 ACM Conference on Fairness, Accountability, and Transparency (FAccT)}, pp. 624--635, 2021.

\bibitem{vonKugelgen2021}
J. von Kügelgen, Y. Gresele, N. Schölkopf.
\newblock Simpson's Paradox in Covid-19 Case Fatality Rates: A Mediation Analysis of Age-Related Causal Effects.
\newblock \textit{IEEE Transactions on Artificial Intelligence}, vol. 2, no. 4, pp. 394--405, 2021.

\bibitem{Hohman2019}
F. Hohman, D. M. Gruen, M. P. G. de la Torre, M. A. L. Vargas.
\newblock The What-If Tool: Interactive Probing of Machine Learning Models.
\newblock \textit{IEEE Transactions on Visualization and Computer Graphics}, vol. 26, no. 1, pp. 53--63, 2019.

\bibitem{tayfailure}
Bitra, S.K.
\newblock Technical Analysis: The Downfall of Microsoft's AI Chatbot Tay.
\newblock \textit{European Journal of Computer Science and Information Technology}, 13(17):1--11, 2025.


\bibitem{gendershades}
Buolamwini, J., Gebru, T.
\newblock Gender Shades: Intersectional Accuracy Disparities in Commercial Gender Classification.
\newblock In \textit{Proceedings of the Conference on Fairness, Accountability, and Transparency}, pp. 77-91. PMLR, 2018.
